...
- 支持可选的 Firebase Google 登录、Apple 登录与退出登录
- 支持登录后将 OTP 账号同步到 Firebase Cloud Firestore
- 支持在上传前使用客户端 AES-GCM 加密账号载荷，并通过独立同步口令派生密钥后缓存到 Keychain
...
运行前还需要在 Firebase 控制台完成以下配置；如果要一次性排掉 `CONFIGURATION_NOT_FOUND`，请按顺序核对：

1. 打开项目后先进入 Authentication 页面，完成 Authentication 的首次初始化
2. 在 Authentication 的 Sign-in method 中启用 Google Provider
3. 在 Authentication 的 Sign-in method 中启用 Apple Provider，并在 Firebase 控制台完成 Apple 服务 ID / Team ID / Key ID / 私钥配置
4. 为 iOS App 使用与工程一致的 Bundle ID，并重新下载匹配当前 Firebase 项目的 GoogleService-Info.plist
5. 创建 Cloud Firestore 数据库
6. 为 Firestore 部署仅允许用户访问自己数据的安全规则
7. 若要联调 Google 登录，确认工程中的 OAuth 回调 scheme 与 Firebase App ID 一致；当前客户端已注册 `app-1-224927712933-ios-4312b74692bb3bfc272f4c`
8. 若切换到别的 Firebase 项目，除了替换 GoogleService-Info.plist，也要同步更新 Info.plist 里的 Firebase OAuth 回调 scheme
9. Apple 登录还需要在 Apple Developer 中为当前 App ID 打开 Sign in with Apple capability，并确保 Xcode 签名团队与 Firebase/Apple 配置一致
...
- 云同步场景中，上传到 Firestore 的账号载荷必须先在客户端加密，再由 Firebase 传输与存储
- 身份登录与云端加密密钥解锁需要分离设计；OAuth 只负责身份识别，同步口令负责派生端到端加密密钥
