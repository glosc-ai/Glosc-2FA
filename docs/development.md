...
- 生物识别锁定状态控制、主题切换与设置持久化
- 账号列表、详情、手动录入、otpauth 导入、二维码扫描导入、编辑和删除
- 列表长按复制验证码、详情页点按或长按验证码复制，并提供统一的复制成功提示
...
- Glosc 2fa/Glosc_2faApp.swift
  负责应用入口、SwiftData 容器注入、偏好对象与安全状态对象装配，并根据应用偏好切换主题。
...
- 详情页点按或长按验证码卡片快速复制，并显示复制成功提示
- 列表页长按账号行快速复制当前验证码，并显示复制成功提示
